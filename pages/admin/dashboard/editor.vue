  ...
  <div class="w-full h-full flex flex-col px-2">
    <div class="w-full flex justify-between py-2">
      <div class="flex gap-2 items-center">
        ...
        <input v-model="title" placeholder="Untitled Post" class="w-64 h-10 p-2 rounded-md outline-none bg-white">
      </div>
      <div class="flex gap-2">
        <Button type="primary" icon-only :ok-loading="okLoading" @click="showTagSelect">
    ...
    <nub-fade>
      <nub-modal v-if="tagSelectVisible" @click-outside="() => tagSelectVisible = false">
        <div class="w-64 p-4 bg-zinc-50">
          <nub-tag-select v-model:selcted="tags" :ok-loading="okLoading" @ok="publishOrPatch" @cancel="() => tagSelectVisible = false" />
        </div>
      </nub-modal>
    </nub-fade>
  </div>
